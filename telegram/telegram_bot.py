...
class TelegramBot:
    ...
    def __init__(self, token: str, access_controller: AccessController, content_manager: ContentManager):
        ...
        self.user_states = {}  # For handling user inputs
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "manage_cat":
            ...
            try:
                await self.generar(update, context)  # Reuse the logic
            except Exception:
                pass
            ...
            parts = data.split("_")
            category = "_".join(parts[2:-1])  # In case category has _
            page = int(parts[-1])
            keyboard = [
            ...
            keyboard = []
            for iid, data in list(idea_dict.items())[:5]:  # Show 5 ideas
                title = data.get('es', {}).get('title', 'Sin título')
                keyboard.append([InlineKeyboardButton(title, callback_data=f"show_idea_{iid}")])
            if page > 0:
            ...
            try:
                await query.answer("Generando idea...")
            except Exception:
                pass
            ...
            en = ideas['en']
            try:
                await query.edit_message_text("Idea generada. Enviando mensajes...")
            except Exception:
                pass
